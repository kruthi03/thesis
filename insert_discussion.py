...
# Text to insert
markdown_text = """2.13 Discussion and Synthesis of Prior Work
...
2.13.1 Semantic Ontologies and Rule-Based Enrichment

In the early development of Security Information and Event Management (SIEM) architectures, researchers focused heavily on augmenting raw alerts with contextual metadata using Semantic Web technologies. Foundational studies, such as the work by Undercoffer, Joshi, and Pinkston (2003), developed target-centric ontologies for intrusion detection systems. They utilized Web Ontology Language (OWL) to map low-level network packets into formalized taxonomy classes. Similarly, the Common Intrusion Detection Framework (CIDF) proposed by Porras et al. (1999) established structural protocols for querying relational asset databases.
...
Weaknesses: These general-domain verification frameworks fell short when applied to highly structured, numerical cybersecurity telemetry. They structurally struggled to distinguish between arithmetically derived truths and falsely precise numerical hallucinations. Furthermore, because these frameworks were designed for open-domain question answering, they were entirely blind to domain-specific logical errors—specifically, the causal misweighting of background context (e.g., assuming an alert triggered because a user is an intern).

Connection to Current Work: This thesis heavily extends the decompose-then-verify paradigm but completely re-architects the verification heuristic. It differs from FActScore and RAGAS by abandoning pure LLM-based NLI evaluation in favor of a dual-signal admission function. This thesis explicitly incorporates deterministic regex matching for numerical grounding and a dedicated spaCy dependency parser to catch the causal misweighting errors that general-domain frameworks systematically miss.

2.13.5 Conformal Prediction Applications in Natural Language

To mathematically bound the heuristic errors of NLI verifiers, recent literature explored Conformal Prediction (Vovk et al., 2005) as a safety mechanism. Specifically, the introduction of Conformal Risk Control (CRC) and the Learn-Then-Test (LTT) framework (Angelopoulos et al., 2021) provided the theoretical machinery to bound non-monotone loss geometries, such as the False Discovery Rate. Recent competitor studies successfully applied these conformal abstention methodologies to question-answering systems, allowing models to selectively abstain from answering when confidence was low.
...
Weaknesses: Existing literature restricted the application of LTT primarily to open-domain classification and question-answering tasks. In the limited instances where component-level selection was applied to generated text, the methodologies often required human gold-reference answers for calibration—a resource that simply does not exist for the millions of novel anomalies generated daily in cybersecurity telemetry.
...
2.13.6 Synthesis and Research Gap
...
if target_idx is not None:
    # We want to insert our new text before chapter 3
    # A simple way with python-docx to insert before a paragraph is using insert_paragraph_before
    ...
    doc.save(r"d:\CP_MTH\kruthika_10001319_master_thesis_draft_updated.docx")
    print("Successfully inserted discussion and saved to new file.")
else:
    print("Could not find 'Chapter 3: Dataset and Signal Construction'")
